		...
		try
		{
			...
			for (int i = 0; i < local_taNodeList.getLength(); i++)
			{
				...
				for (int j = 0; j < week_local_taNodeList.getLength(); j++)
				{
					...
					System.out.printf("\n- %d 주차 평년 : %s%n%n",(j+1),XMLDOM.getText(week_local_taElement, "week"+(j+1)+"_local_ta_normalYear")
							.replaceAll(" ","").replaceAll("\n", ""));
					System.out.printf("\n- %d 주차 평년비슷범위 : %s%n%n",(j+1),XMLDOM.getText(week_local_taElement, "week"+(j+1)+"_local_ta_similarRange")
							.replaceAll(" ","").replaceAll("\n", ""));
					System.out.printf("\n- %d 주차 예보 확률 (낮음) : %s%n%n",(j+1),XMLDOM.getText(week_local_taElement, "week"+(j+1)+"_local_ta_minVal")
							.replaceAll(" ","").replaceAll("\n", ""));
					System.out.printf("\n- %d 주차 예보 확률 (비슷) : %s%n%n",(j+1),XMLDOM.getText(week_local_taElement, "week"+(j+1)+"_local_ta_similarVal")
							.replaceAll(" ","").replaceAll("\n", ""));
					System.out.printf("\n- %d 주차 예보 확률 (높음) : %s%n%n",(j+1),XMLDOM.getText(week_local_taElement, "week"+(j+1)+"_local_ta_maxVal")
							.replaceAll(" ","").replaceAll("\n", ""));
